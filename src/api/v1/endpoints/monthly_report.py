...
from models.monthly_report import MonthlyReportCreate, MonthlyReportCreateRequest
...
async def create_month_report(
    ...
    emotion_id = await diaries_cruds.get_most_common_emotion_for_month(
        session=session, user_id=current_user.id, search_date_yymm=create_date_yymm
    )
    ...
    monthly_report = MonthlyReportCreate(
        user_id=current_user.id, content=result, created_date_yymm=create_date_yymm
    )
...
async def get_monthly_report(
    session: SessionDep, current_user: CurrentUser, created_date: str
):
    monthly_report = await monthly_report_crud.get_monthly_report_by_date(
        session=session, user_id=current_user.id, created_date_yymm=created_date
    )
